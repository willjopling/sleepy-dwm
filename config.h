...
static const Key keys[] = {
	...
	{0,	     XF86XK_AudioLowerVolume,	   volset,	   {.v = "-5%"  } },
	{0,	     XF86XK_AudioRaiseVolume,	   volset,	   {.v = "+5%"  } },
...
#define CMDSIZE 1024
#define STATUSCMD "slstatus"

void volset(const Arg *arg) {

	Arg a;
	const char *setvolcmd[] = { "/usr/bin/pactl", "set-sink-volume", "0", arg->v, NULL };
	a.v = setvolcmd;
	spawn(&a);

	char line[CMDSIZE];
	FILE *cmd = popen("pidof " STATUSCMD, "r");

	fgets(line, CMDSIZE, cmd);
